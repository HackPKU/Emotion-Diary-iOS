    ...
    if (indexPath.section == 0) {
        ...
        if ([ActionPerformer hasLoggedIn]) {
            if (indexPath.row == 0) {
                ...
                cell.labelName.text = [USER_DEFAULTS objectForKey:USER_NAME];
...
- (void)reloadUserInfo {
    if ([ActionPerformer hasLoggedIn]) {
        [ActionPerformer viewUserWithName:[USER_DEFAULTS objectForKey:USER_NAME] andBlock:^(BOOL success, NSString * _Nullable message, NSDictionary * _Nullable dataUserInfo) {
            if (!success) {
                return;
            }
            [USER_DEFAULTS setValuesForKeysWithDictionary:@{USER_INFO: dataUserInfo}];
